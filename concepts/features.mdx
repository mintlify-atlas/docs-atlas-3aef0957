...
<CardGroup cols={2}>
  ...
  <Card title="volume" icon="volume-high">
    `volume` (0–1) and `muted` state, `setVolume()` and `setMuted()` actions. Includes `volumeAvailability`.
  </Card>
  <Card title="time" icon="clock">
    `currentTime` and `duration` state, `seekTo()` action.
  </Card>
  <Card title="fullscreen" icon="expand">
    `fullscreen` state and `enterFullscreen()` / `exitFullscreen()` actions. Includes `fullscreenAvailability`.
  </Card>
  <Card title="pip" icon="picture-in-picture">
    Picture-in-picture state and `enterPiP()` / `exitPiP()` actions. Includes `pipAvailability`.
  </Card>
  <Card title="textTracks" icon="closed-captioning">
    Subtitle and caption track selection.
  </Card>
